...
description: >
  ...
  산출물은 CSV 원자료와, 요약 탭·서식을 갖춘 xlsx 워크북 두 가지를 기본
  제공한다. 재무·부문 데이터를 엑셀이나 CSV 파일로 요청받았거나,
...
- 스크립트가 만든 CSV(UTF-8 BOM)와, 그것을 Anthropic 기본 제공 xlsx
  스킬(`Skill("xlsx")`)로 조판한 .xlsx 워크북을 **둘 다** 전달한다. 두 시트를
  함께 요청받았으면 xlsx는 한 파일에 탭으로 합친다.
- 사용자가 형식을 하나만 지정하면 그 형식만 전달한다.
- xlsx 스킬이 없는 환경이면 CSV만 전달하고 그 사실을 안내한다 — 형식을 이유로
  데이터 생성을 줄이거나 미루지 않는다.

### xlsx 조판 규칙 — 실사에서 바로 쓰는 워크북으로

xlsx는 CSV의 기계적 미러가 아니라 분석 워크북으로 조판한다.

- **탭 구성**: 맨 앞에 `00_요약` 탭을 모델이 작성한다 — 회사·기준일·포함 보고서
  목록(접수번호), 핵심 관찰(재작성 징후·스크립트 경고·커버리지 공백), 각 탭 안내.
  Raw 탭은 그 뒤에 `01_Raw_BSPL` · `02_Raw_부문별매출` 순으로 둔다 (하나만
  요청받았으면 그 탭만).
- **모든 탭 공통 서식**: 첫 행 제목 + 기준일·출처 메타 블록, 헤더 행 틀고정,
  헤더 굵게·채움, 금액 열 천단위 숫자서식, 읽을 만한 열 너비.
- **Raw 탭 값 불변**: 데이터 셀의 값과 행 구조는 CSV와 1:1로 같아야 한다.
  서식은 표시 계층에만 입히고, 값 변경·행 재배열·수식 삽입·추정 보간은 금지.
  조판 뒤 CSV와 전 셀 대조로 검증하고 그 결과를 보고에 명시한다.
- **요약 탭의 유도값**: 합계 등을 넣으려면 실제 엑셀 수식으로 넣거나 계산된
  값을 넣는다. `=SUM(...)` 모양의 **텍스트**를 수식처럼 흉내 내는 것은 금지.
