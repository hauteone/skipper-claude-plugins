...
파라미터를 좁혀 가져와 표로 정리해 전달하고, "CSV 자동 내보내기는 스크립트가
실행되는 환경(Claude Code·Cowork)에서만 가능하다"고 명시한다.

## 인증

스크립트가 REST API를 직접 호출하므로 세션에 둘 중 하나가 있어야 한다:

- `SKIPPER_API_KEY` — 정적 키(만료 없음, 반복 실행에 권장).
  `support@skipperlabs.ai`에서 발급.
- `SKIPPER_ACCESS_TOKEN` — 단기 액세스 토큰(만료됨). MCP 커넥터가 연결돼
  있으면 `mint_script_token` 도구로 즉시 발급받아, 스크립트 실행과 같은 셸
  명령 안에서 `SKIPPER_ACCESS_TOKEN="<값>"`으로 함께 넘긴다.
  `SKIPPER_API_KEY`가 있으면 무시된다.

`... 환경변수가 비어 있습니다` 오류가 나면 위 순서대로 안내한다.
