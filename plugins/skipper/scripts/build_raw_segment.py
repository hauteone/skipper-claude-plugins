...
METRIC_NAMES = {
    ...
    "goodwill": "영업권", "depreciation": "감가상각비", "amortisation": "무형자산상각비",
    "other": "기타",
}
...
def fetch_via_series(company: str, reports: int) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    """segment_series 한 번으로 보고서 섹션과 XBRL 팩트를 모두 가져온다."""
    ...
    return picked, (data.get("xbrl_facts") or {}).get("rows") or []
...
def facts_rows(rows: list[dict[str, Any]]) -> list[list[Any]]:
    ...
    out: list[list[Any]] = [
        ["사업연도", "보고서", "기간유형", "축", "부문/지역/제품", "지표", "금액(백만원)"]
    ]
    def key(r: dict[str, Any]) -> tuple:
        return (str(r.get("bsns_year") or ""), str(r.get("reprt_code") or ""),
                str(r.get("axis_type") or ""), str(r.get("member") or ""),
                str(r.get("metric") or ""))
    ...
    for r in sorted(rows, key=key, reverse=True):
        ...
        out.append([
            r.get("bsns_year", ""),
            r.get("period_type") or r.get("reprt_code", ""),
...
def main() -> None:
    ...
    source = "segment_series"
    try:
        reports, facts = fetch_via_series(company["ticker"], args.reports)
    except SkipperError as exc:
        print(f"segment_series 사용 불가 ({exc}) — 공시 원문 조회로 폴백합니다.", file=sys.stderr)
        source = "list_disclosures + get_document"
        reports = fetch_via_documents(company["ticker"], args.reports)
        try:
            facts = (call("segment_facts", query=company["ticker"]) or {}).get("rows") or []
        except SkipperError:
            facts = []
